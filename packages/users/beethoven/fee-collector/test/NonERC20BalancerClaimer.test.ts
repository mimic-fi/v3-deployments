...
describe('NonERC20BalancerClaimer', () => {
  ...
  describe('initialization', async () => {
    it('cannot call parent initialize', async () => {
      await expect(
        task.initializeBalancerClaimer(
          buildEmptyTaskConfig(owner, smartVault),
    ...
    it('has a withdrawer reference', async () => {
      expect(await task.protocolFeeWithdrawer()).to.be.equal(protocolFeeWithdrawer.address)
    })

    it('has a collector reference', async () => {
      expect(await task.protocolFeesCollector()).to.be.equal(protocolFeesCollector.address)
    })
  })
  ...
  describe('BalancerClaimer', () => {
    ...
    describe('setProtocolFeeWithdrawer', () => {
      context('when the sender is authorized', () => {
        ...
        context('when the given address is zero', () => {
          it('reverts', async () => {
            await expect(task.setProtocolFeeWithdrawer(ZERO_ADDRESS)).to.be.revertedWith(
              'TaskProtocolFeeWithdrawerZero'
            )
          })
        })
      })
    ...
    describe('setProtocolFeesCollector', () => {
      context('when the sender is authorized', () => {
        ...
        context('when the given address is zero', () => {
          it('reverts', async () => {
            await expect(task.setProtocolFeesCollector(ZERO_ADDRESS)).to.be.revertedWith(
              'TaskProtocolFeesCollectorZero'
            )
          })
        })
      })
          ...
          context('when the amount is greater than zero', () => {
            ...
            const itExecutesTheTaskProperly = (requestedAmount: BigNumberish) => {
              ...
              it('transfers the token in from the protocol fee withdrawer to the smart vault', async () => {
                ...
                const amountAfterFee = bn(transactedAmount).mul(fp(1).sub(fee)).div(fp(1))
            ...
            context('with balance connectors', () => {
              ...
              it('updates the balance connectors properly', async () => {
                ...
                await assertIndirectEvent(tx, smartVault.interface, 'BalanceConnectorUpdated', {
                  id: nextConnectorId,
                  token,
                  amount: totalBalance.mul(fp(1).sub(fee)).div(fp(1)),
                  added: true,
                })
              })
            })
