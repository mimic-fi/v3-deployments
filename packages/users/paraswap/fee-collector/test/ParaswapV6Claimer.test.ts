...
describe('ParaswapV6Claimer', () => {
  let task: Contract, smartVault: Contract, authorizer: Contract, feeVault: Contract
  ...
  beforeEach('deploy task', async () => {
    feeVault = await deploy('AugustusFeeVaultMock')
    task = await deployProxy(
      'ParaswapV6Claimer',
      [],
      [buildEmptyTaskConfig(owner, smartVault), feeVault.address],
      'initializeParaswapV6Claimer'
    )
  })

  describe('execution type', () => {
    it('defines it correctly', async () => {
      const expectedType = ethers.utils.solidityKeccak256(['string'], ['PARASWAP_V6_CLAIMER'])
      expect(await task.EXECUTION_TYPE()).to.be.equal(expectedType)
    })
  })

  describe('setFeeVault', () => {
    context('when the sender is authorized', () => {
      beforeEach('set sender', async () => {
        const setFeeVaultRole = task.interface.getSighash('setFeeVault')
        await authorizer.connect(owner).authorize(owner.address, task.address, setFeeVaultRole, [])
        task = task.connect(owner)
      })

      context('when the given address is not zero', () => {
        it('sets the fee vault', async () => {
          await task.setFeeVault(other.address)

          expect(await task.feeVault()).to.be.equal(other.address)
        })

        it('emits an event', async () => {
          const tx = await task.setFeeVault(other.address)

          await assertEvent(tx, 'FeeVaultSet', { feeVault: other })
        })
      })

      context('when the given address is zero', () => {
        it('reverts', async () => {
          await expect(task.setFeeVault(ZERO_ADDRESS)).to.be.revertedWith('TaskFeeVaultZero')
        })
      })
    })
    ...
    context('when the sender is not authorized', () => {
      ...
      it('reverts', async () => {
        await expect(task.setFeeVault(other.address)).to.be.revertedWith('AuthSenderNotAllowed')
      })
    })
  })
    ...
    context('when the sender is authorized', () => {
      ...
      context('when the token to claim is not the address zero', () => {
        ...
        context('when the amount is greater than zero', () => {
          ...
          beforeEach('fund fee vault', async () => {
            await token.mint(feeVault.address, totalBalance)
          })
          ...
          const itExecutesTheTaskProperly = (requestedAmount: BigNumberish) => {
            ...
            it('calls the call primitive', async () => {
              ...
              const data = feeVault.interface.encodeFunctionData('collect', [
                token.address,
                transactedAmount,
                smartVault.address,
                smartVault.address,
            ...
            it('transfers the token in from the fee vault to the smart vault', async () => {
              const previousSmartVaultBalance = await token.balanceOf(smartVault.address)
              const previousFeeVaultBalance = await token.balanceOf(feeVault.address)
              ...
              const currentFeeVaultBalance = await token.balanceOf(feeVault.address)
              expect(currentFeeVaultBalance).to.be.eq(previousFeeVaultBalance.sub(transactedAmount))
            })
