...
const script = Script.forForkedNetwork('connectors/balancer-v2-pool/v1.0.0', hre)
...
test(script.id, () => {
  ...
  before('run script', async () => {
    await script.run()
    balancerConnector = await script.deployedInstance('BalancerV2PoolConnector')
  })

  it('registers the balancer v2 pool connector in the registry correctly', async () => {
