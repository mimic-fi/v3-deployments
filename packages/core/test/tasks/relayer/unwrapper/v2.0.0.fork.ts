...
const script = Script.forForkedNetwork('tasks/relayer/unwrapper/v2.0.0', hre)
...
test(script.id, () => {
  ...
  before('run script', async () => {
    await script.run()
    unwrapper = await script.deployedInstance('UnwrapperRelayerFunder')
  })

  it('registers the unwrapper relayer funder in the registry correctly', async () => {
